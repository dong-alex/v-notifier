...
const GetContactButton = (name: string, phone: string, handler: (number: string) => void, hasSentMessage: boolean) => {
  return (
    <button
    ...
    >
      <span className="text-blue-500">{name}{hasSentMessage && " ✅"}</span>
    </button>
  );
}
  ...
  return (
    <div className="overflow-y-auto my-4 mr-8 max-h-96 p-2">
      {contactArray.map(({ name, phone }, i) => {
        const hasSentMessage = sentContacts.has(phone);
        return GetContactButton(name, phone, contactHandler, hasSentMessage);
      })}
    </div>
  )
};
